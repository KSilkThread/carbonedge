...
def main():
    ...
    "command",'''docker run --rm --name tools
    ...
    changesForARM2 = yA.yamlChange(f"{RELATIVE_PATH_TO_MINIFAB_MODULE}/playbooks/ops/consoleup/apply.yaml",["name"],["Start configtxlator"],"command",'''docker run -d --network {{ NETNAME }} --name configtxlator.{{ NETNAME }} --hostname configtxlator.{{ NETNAME }}
    ...
    changesForARM3 = yA.yamlChange(f"{RELATIVE_PATH_TO_MINIFAB_MODULE}/playbooks/ops/netup/dockerapply.yaml",["name"],["Start couchdb nodes if db type is set to couchdb"],"command",'''docker run -d --network {{ NETNAME }} --name {{ item.fullname }} {{ item.portmap }}
    ...
    changesForARM4 = yA.yamlChange(f"{RELATIVE_PATH_TO_MINIFAB_MODULE}/playbooks/ops/netup/dockerapply.yaml",["name"],["Start all peer nodes"],"command",'''docker run -d --network {{ NETNAME }} --name {{ item.fullname }} --hostname {{ item.fullname }}
    ...
    changesForARM5 = yA.yamlChange(f"{RELATIVE_PATH_TO_MINIFAB_MODULE}/playbooks/ops/netup/dockerapply.yaml",["name"],["Start all orderer nodes"],"command",'''docker run -d --network {{ NETNAME }} --name {{ item.fullname }} --hostname {{ item.fullname }}
    --env-file {{ pjroot }}/vars/run/{{ item.fullname }}.env {{ item.portmap }}
    ...
    changesForARM6 = yA.yamlChange(f"{RELATIVE_PATH_TO_MINIFAB_MODULE}/playbooks/ops/netup/dockerapply.yaml",["name"],["Start all ca nodes"],"command",'''docker run -d --network {{ NETNAME }} --name {{ item.fullname }} --hostname {{ item.fullname }}
    --env-file {{ pjroot }}/vars/run/{{ item.fullname }}.env {{ item.portmap }}
    ...
    changesForARM7 = yA.yamlChange(f"{RELATIVE_PATH_TO_MINIFAB_MODULE}/playbooks/ops/netup/dockerapply.yaml",["name"],["Start cli container for all fabric operations"],"command",'''docker run -dit --network {{ NETNAME }} --name {{ CLINAME }} --hostname {{ CLINAME }}
    ...
    changesForARM8 = yA.yamlChange(f"{RELATIVE_PATH_TO_MINIFAB_MODULE}/playbooks/ops/netup/k8sapply.yaml",["name","block","name"],["Start cli container for all fabric operations", None ,"start cli container"],"command",'''docker run -dit --network {{ NETNAME }} --name {{ CLINAME }} --hostname {{ CLINAME }}
      -v /var/run/docker.sock:/var/run/docker.sock
      -v {{ hostroot }}/vars:/vars
      -v {{ hostroot }}/vars/chaincode:{{ gopath }}/src/github.com/chaincode
