...
@Contract(
        ...
        info = @Info(
                ...
                description = "A measurement contract with main focus on high throughput",
                version = "0.0.1-Alpha",
                license = @License(
...
public class MonitoringContract implements ContractInterface {
    ...
    /** 
     * Submits a MeasurementAsset to the ledger 
     * 
     * @param context Hyperledger fabric context
     ...
     */
    @Transaction(intent = Transaction.TYPE.SUBMIT)
    public String pushMeasurementAsset(final Context context, final String sensorid, final int data){
        ...
        Response response = stub.invokeChaincodeWithStringArgs("certificateCC", List.of("isValid", sensorid, cmspID), stub.getChannelId());
        ...
        MeasurementAsset asset = new MeasurementAsset(sensorid, cmspID, data, timestamp);
        stub.putStringState(key.toString(), asset.toJSON());
        return helper.createSuccessResponse("MeasurementAsset: "+ asset.toJSON() + "added successfully!");
    }

    
    /** 
     * Checks, if a specic MeasurementAsset exists on the ledger
     * 
     * @param context Hyperledger fabric context
    ...
    /**
     * Queries a specific MeasurementAsset
     * 
     * @param context Hyperledger fabric context
     * @param mspid Mspid of the organisation, which owns the sensor
     * @param sensorid Organisation wide unique sensor id
     ...
     */
    @Transaction(intent = Transaction.TYPE.EVALUATE)
    public String readMeasurementAsset(Context context, final String mspid, final String sensorid, final String timestamp){
    ...
    /**
     * Gets all MeasurementAssets, which belong to one specific organisation
     *  
     * @param context Hyperledger fabric context
     * @param mspid Mspid of the organisation, which owns the sensor
     * @return String Response
    ...
    /**
     * Queries all MeasurementAssets, which belong to one specific organisation and were invokey by a specific sensor 
     * 
     * @param context Hyperledger fabric context
     * @param mspid Mspid of the organisation, which owns the sensor
     * @param sensorid Organisation wide unique sensor id
    ...
    public String querySensorEntries(final Context context, final String mspid, final String sensorid){
        ...
        for(KeyValue kv: iterator){
            assetList.add(MeasurementAsset.fromJSON(kv.getStringValue()));
        }
